...
class ReadLenDataset(TensorDataset):
    def __init__(self, path):
        with np.load(path) as arr:
            ...
            read_length_norm = torch.from_numpy(arr["read_length_norm"].astype(np.float32)).unsqueeze(1)
            log_read_length_norm = torch.from_numpy(arr["log_read_length_norm"].astype(np.float32)).unsqueeze(1)
...
class CNNV3ReadLen(nn.Module):
    def __init__(self, embedding_dim=12, conv_channels=64, dropout=0.3):
        super().__init__()
    ...
    def forward(self, ref_kmer, query_kmer, pos_norm, read_length_norm, log_read_length_norm, ref_strand, ref_mod_strand):
        ...
        scalar = torch.cat(
            [pos_norm, read_length_norm, log_read_length_norm, ref_strand, ref_mod_strand],
            dim=1,
        )
        scalar = self.scalar_branch(scalar)
        return self.classifier(torch.cat([pooled, scalar], dim=1)).squeeze(1)
...
def run_epoch(model, loader, criterion, device, optimizer=None):
    ...
    for ref_kmer, query_kmer, pos_norm, read_length_norm, log_read_length_norm, ref_strand, ref_mod_strand, labels in loader:
        ref_kmer = ref_kmer.to(device)
        query_kmer = query_kmer.to(device)
        pos_norm = pos_norm.to(device)
        read_length_norm = read_length_norm.to(device)
        log_read_length_norm = log_read_length_norm.to(device)
        ...
        logits = model(ref_kmer, query_kmer, pos_norm, read_length_norm, log_read_length_norm, ref_strand, ref_mod_strand)
...
def main():
    ...
    train_ds = ReadLenDataset(args.train_npz)
    val_ds = ReadLenDataset(args.val_npz)

    train_loader = DataLoader(train_ds, batch_size=args.batch_size, shuffle=True)
    val_loader = DataLoader(val_ds, batch_size=args.batch_size, shuffle=False)

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = CNNV3ReadLen(args.embedding_dim, args.conv_channels, args.dropout).to(device)
    ...
    config["extra_scalar_features"] = ["read_length_norm", "log_read_length_norm"]
