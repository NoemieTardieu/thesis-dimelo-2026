...
def parse_tracks(entries: List[str]) -> List[Tuple[str, Path]]:
    tracks: List[Tuple[str, Path]] = []
    for entry in entries:
        if '=' not in entry:
            raise SystemExit(f'Expected LABEL=PATH, got {entry!r}')
        label, path = entry.split('=', 1)
        tracks.append((label.strip(), Path(path.strip()).expanduser()))
    return tracks
...
def main() -> None:
    p = argparse.ArgumentParser(description='Plot average centered profile for one BED over one or more bigWig tracks.')
    p.add_argument('--regions-bed', type=Path, required=True)
    p.add_argument('--track', action='append', required=True, metavar='LABEL=PATH')
    p.add_argument('--out', type=Path, required=True)
    p.add_argument('--title', default='Average profile')
    ...
    if not regions:
        raise SystemExit(f'No regions loaded from {args.regions_bed}')
    ...
    for idx, (label, path) in enumerate(tracks):
        if not path.exists():
