...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train a simple MLP baseline on balanced NPZ subsets.")
...
def load_npz(path: str | Path) -> TensorDataset:
    with np.load(path) as arr:
        ref = arr["ref_kmer"].astype(np.float32) / 4.0
        query = arr["query_kmer"].astype(np.float32) / 4.0
        ...
        x = np.concatenate([ref, query, scalar], axis=1)
...
def main() -> None:
    ...
    train_ds = load_npz(args.train_npz)
    val_ds = load_npz(args.val_npz)
